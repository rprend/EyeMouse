...
int main(int argc, char **argv)
{
  ...
  if (!faceCascade.load("../data/haarcascades/haarcascade_frontalface_alt.xml"))
  {
      std::cerr << "Could not load face detector." << std::endl;
      return -1;
  }
  if (!eyeCascade.load("../data/haarcascades/haarcascade_eye_tree_eyeglasses.xml"))
  {
  ...
  while (1)
  {
      ...
      if (cv::waitKey(30) == 27) break;  // takes 30 frames per second. if the user presses any button, it stops from showing the webcam
  }
  return 0;
}
